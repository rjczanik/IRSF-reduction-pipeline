...
def stardetection(image_loc):
	c_vmin, c_vmax = np.log10(550.), np.log10(850.)
	global FWHM
	FWHM = det_area = 5
...
#Plotting the raw image
	...
	ax1.set_title('(a) The raw scanned image')
	ax1.imshow(plot_image, origin='lower', cmap='gray', vmin=c_vmin, vmax=c_vmax)
	plt.draw()
	plt.show()
...
#Drawing the stars that were detected with Source Extractor as a whole collection in the candidates array
	ax2.imshow(plot_image, cmap = plt.cm.binary, origin='lower', vmin=c_vmin, vmax=c_vmax)
	ax2.set_title('(b) Stars detected with SEXTRACTOR')
	ax2.scatter(starsx, starsy, marker='+')
	plt.draw()
	plt.show()
...
#			print dy, image[candidates[i,1]-1,candidates[i,0]-1]-image[candidates[i,1]-1-dy,candidates[i,0]-1-dy], (candidates[ i, 2 ] - image[ candidates[ i, 1 ]-1 + profile_range, candidates[ i, 0 ]-1 + profile_range ])/2, candidates[i,2] - image[candidates[i, 1]-1 - profile_range, candidates[i,0]-1 - profile_range]
	...
	ax3.set_title('(c) FWHM candidates') #Plotting the stars that were used for the FWHM selection and the construction of the PSF
	ax3.imshow(plot_image, cmap = plt.cm.binary, origin='lower', vmin=c_vmin, vmax=c_vmax)
	ax3.scatter(candidates[:,0],candidates[:,1], color='orange', marker='+')
	plt.draw()
	plt.show()
...
#########################This is for DAOFIND################################
	...
	iraf.daophot.daofind( datedir+runnumber+filter+fitsname, output = datedir+runnumber+filter+fitsname+'.coo', starmap = datedir+runnumber+filter+fitsname+'.starmap.', skymap = datedir+runnumber+filter+fitsname+'.skymap.', boundary = 'nearest', constant = 0.0, interactive = 'no', wcsout = ')_.wcsout', cache = 'no', verify = 'yes', update = 'yes', verbose = 'yes' )
...
#    print str(len(starsx))+' good sources'
	...
	ax4.set_title('(d) Stars found with DAOFIND') #Plotting the number of stars that were detected with DAOFIND, using the FWHM from the candidates
	plt.xlabel('X')
	plt.ylabel('Y')
	plt.imshow(plot_image, cmap = plt.cm.binary, origin='lower', vmin=c_vmin, vmax=c_vmax)
	plt.scatter(starsx, starsy, marker='+')
...
#    if OverWrite == 1 or not os.path.exists(DAOParsFileName) :
	...
	iraf.daopars.setParam('fitrad',0.9*FWHM)#0.8*FWHM
	...
	iraf.daopars.setParam('sannulus',FWHM+2.0)#FWHM+1.5
...
#    iraf.daophot.phot.setParam('fitskypars',)
	...
	iraf.daophot.phot(datedir+runnumber+filter+fitsname,coords=datedir+runnumber+filter+fitsname+'.coo',output=datedir+runnumber+filter+fitsname+'.mag', interactive = 'no')
...
#        os.remove(datedir+runnumber+filter+fitsname+'.mag')
	iraf.daophot.phot(datedir+runnumber+filter+fitsname,coords=datedir+runnumber+filter+fitsname+'.brt_src.coo',output=datedir+runnumber+filter+fitsname+'.brt_src.mag', interactive = 'no')
...
#    else:
	iraf.daophot.pstselect(datedir+runnumber+filter+fitsname,photfile=datedir+runnumber+filter+fitsname+'.brt_src.mag',pstfile=datedir+runnumber+filter+fitsname+'.pst',verbose='yes', maxnpsf=canstars - 5, interactive = 'no')
...
#    plt.scatter(pstdata[:,1], pstdata[:,2], c='r', marker='d', linewidths=2.0)
	...
	iraf.daophot.nstar(datedir+runnumber+filter+fitsname, groupfile=datedir+runnumber+filter+fitsname+'.psg', psfimage=datedir+runnumber+filter+fitsname+'.psf.fits', nstarfile=datedir+runnumber+filter+fitsname+'.nst',rejfile=datedir+runnumber+filter+fitsname+'.brt_src.nrj',update='yes', cache = 'no', verify = 'yes')
	print 'Subtracting the PSF stars that were calculated with nstar'
	iraf.daophot.substar(datedir+runnumber+filter+fitsname,photfile=datedir+runnumber+filter+fitsname+'.nst',psfimage=datedir+runnumber+filter+fitsname+'.psf.fits',subimage=datedir+runnumber+filter+fitsname+'.sub.fits',cache='no',update='yes',verbose='yes', verify = 'yes')#,exfile=datedir+runnumber+filter+fitsname+'.opst'
	print 'Using the calculated PSF on all of the detected stars on the science frame'
	iraf.daophot.allstar(datedir+runnumber+filter+fitsname,photfile=datedir+runnumber+filter+fitsname+'.mag',psfimage=datedir+runnumber+filter+fitsname+'.psf.fits',allstarfile=datedir+runnumber+filter+fitsname+'.als',rejfile=datedir+runnumber+filter+fitsname+'.nrj',subimage=datedir+runnumber+filter+fitsname+'.als.sub.fits',cache='no',verify='yes',update='yes',verbose='yes')
...
#Photometry on the newly detected faint sources from the original image
	...
	iraf.daophot.phot(datedir+runnumber+filter+fitsname, coords=datedir+runnumber+filter+fitsname+'.fnt_src.coo',output=datedir+runnumber+filter+fitsname+'.fnt_src.mag', interactive = 'no')	
...
#Performing photometry on the combined 'stars' file
	iraf.daophot.allstar(datedir+runnumber+filter+fitsname,photfile=datedir+runnumber+filter+fitsname+'.allsrcs.mag',psfimage=datedir+runnumber+filter+fitsname+'.psf.fits',allstarfile=datedir+runnumber+filter+fitsname+'.allsrcs.als',rejfile=datedir+runnumber+filter+fitsname+'.allsrcs.nrj',subimage=datedir+runnumber+filter+fitsname+'.allsrcs.als.sub.fits',cache='no',verify='yes',update='yes',verbose='yes')
...
#DAOPARS ###############################################################
	...
	iraf.daophot.phot(filename,coords=filename+'.coo',output=filename+'.mag', interactive = 'no', update ='no', cache= 'no', verify= 'yes')
...
#		img_list.write(location+'kPers'+star+'n01.fits\n')
	...
	iraf.mknobsfile('@stnd_list.list', idfilters='J,H,Ks', wrap='Yes',allfilters='No',verify='Yes',verbose='Yes', observations='stnd_out.list', imsets='stnd_img.list')
	os.chdir('../../../code/python/')
...
#	iraf.mkconfig(config=location+'irsf.cfg', catalog='Persson.dat', observations=location+'stnd_out.list', transform = 'tPersson.dat', catdir='', verify='Yes', edit='Yes', check='Yes',verbose='Yes') #Creating the configure file that states which variables correspnd to which quantities in the transformation equations to the standard system
	...
	iraf.fitparams.setParam('interactive','no')
...
pyr.iraf.set(min_lenuserarea	= 500000)
datedir = '../../nir/mosaic2014/'
runnumber = ''
fitsname = 'extendedmosaic.fits'
#filter = 'j'
comment = ''#'extendedmosaic.fits'
#FWHM = 0
standard_name1 = '9129'
standard_name2 = '9132'
standard_name3 = '9136'
...
for filter in fltrs:
	...
	if ( filter == 'h' ):
		...
		himage.suptitle('The images for the sources that were detected in the H band')
                plt.savefig(datedir+filter+'detection_plots.pdf', format='PDF', dpi=600)
	if ( filter == 'k' ):
		plt.figure()
